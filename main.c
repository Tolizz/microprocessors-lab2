...
Queue rx_queue; //UART receive queue
...
volatile bool buffer_full_warned = false;
...
bool is_profile_valid(const char* str){
	int len = strlen(str);
	...
	while(1) {
		...
		if(queue_dequeue(&rx_queue, &rx_char)){
			if (currentState == STATE_ESTOP){
				...
				if (rx_char == '\r' || rx_char == '\n'){
					...
					if(rx_index > 0){
						rx_buffer[rx_index] = '\0'; // Null terminated string
						profile_ready = true;					
						rx_index = 0;								// Reset for next input
						stop_timeout_timer();				// Stop timeout timer since Enter was pressed
					}
				} else if (rx_char == '\b' || rx_char == 0x7F){ // backspace handling
					buffer_full_warned = false;
					if (rx_index > 0) {
						rx_index--;									// delete last character from buffer
						uart_print("\b \b");				// Erase character visual from terminal
					}
				} else {
					if (rx_index == 0) start_timeout_timer(4);
					// buffer full handling
					if (rx_index < BUFF_SIZE - 1){
						rx_buffer[rx_index++] = (char)rx_char;
						uart_tx(rx_char);
					} else {
						if (!buffer_full_warned) {
							buffer_full_warned = true;
							
							uart_print("\r\n[WARNING] Buffer full! Press Enter.\r\n");
		...
		switch (currentState) {
			...
			case STATE_IDLE:
				if (profile_ready) {
					profile_ready = false;
					if (is_profile_valid((char*)rx_buffer)){
						...
						if (current_profile[profile_length - 1] == '-') {
							...
							start_digit_timer(); // Start 2-second switching
								...
								uart_print("\r\n[ERROR] Invalid profile! Use only digits and '-' at the end for loop. \r\n");
								uart_print("Enter profile: ");
							}
				}
				break;
			...
			case STATE_EXECUTING:
				...
				if (profile_ready) {
					profile_ready = false;
					// Validate new profile before interrupting cuurent one
					if (is_profile_valid((char*)rx_buffer)) {
						stop_digit_timer();
						set_led_frequency(0);
						currentState = STATE_IDLE; 
